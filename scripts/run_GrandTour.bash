...
DATASET_BAG_PATH=$1
WS_PATH="/home/ws"
GRANDTOUR_CONFIG_PATH="${WS_PATH}/src/VINS-Fusion/config/GrandTour"
...
for bag_file in ${DATASET_BAG_PATH}/*.bag; do
    ...
    if [ -d "${WS_PATH}/output/pose/VINS-Fusion_StereoIMU/${bag_name}" ] || [ -d "${WS_PATH}/output/time/VINS-Fusion_StereoIMU/${bag_name}" ]; then
        echo "Stereo IMU output already exists. Skipping ..."
    else
        is_mono_imu=false
        config_file=${GRANDTOUR_CONFIG_PATH}/stereo_imu_config.yaml
